...
import { InputEnum } from '../screens/Index'
    ...
    <>
      <div key={tool.id} className='h-48 group relative rounded-md flex flex-col justify-between shadow-slate-900 shadow-md p-4 bg-gradient-to-r from-slate-800 to-slate-700'>
        ...
        />
        {
          isEdit ?
            <>
              < CheckIcon onClick={handleUpdate} className="h-6 w-6 text-green-500 absolute top-4 right-12 cursor-pointer mx-2" />
              <XCircleIcon onClick={onClose} className="h-6 w-6 text-red-900 absolute top-4 right-4 cursor-pointer" />
            </> :
            <button className='btn btn-active btn-ghost hidden group-hover:block absolute top-0 right-3 p-0' onClick={toggleIsEdit}>
